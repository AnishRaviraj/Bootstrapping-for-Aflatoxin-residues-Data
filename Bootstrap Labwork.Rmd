...
deviation σ = 8.
a) The sampling distribution of 𝑋̅ for samples of size 500 will have what
distribution, mean and standard error?

ans: For a quantitative variable with population mean µ and std. deviation σ with sample mean �, the sampling distribution spread will be  σ/sq. root(n) and the distribution will be normal as long as the dataset population in normal. For this question, it will be normally distributed with a sample of size 500, mean = 36, and std. error = 8/sq.root(500).
...
compute an 80% percentile confidence interval for θ.
```{r}
set.seed(17)
...
```
80% percentile confidence interval for θ is between 5.179 and 5.432
